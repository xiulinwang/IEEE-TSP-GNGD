...
for snr = 16:20
    ...
    for numm = 1:mont
        ...
        cd ..
        cd DATA
        [~, ~, ~, JNJD_Ten, ~, AA, info] = f_EXP2_DATA(N,R,K,snr);
        cd ..
        cd GJD_FUNCS
        %% Separate
        tic
        [W(:,:,numm), Y1, sub_off, sum_off, sub_diag, sum_diag] = f_JNJD(JNJD_Ten, R); 
        time_(numm) = toc;
        [ISI(numm,:),J_ISI(numm)] = f_ISI(AA,W(:,:,numm));        
        temp  = sum_off./sum_diag;
        off{numm} = temp;
        fprintf('Running time: %ds \n',time_(numm));
        fprintf('----------------------------------\n');        
    end
    cd ..
    cd EXP2_APPRO_GJD
    cd RESULTS
    file_name = ['EXP2_JNJD_SNR=' num2str(snr) 'dB_N=' num2str(N) '_R=' num2str(R) '_K=' num2str(K) '.mat'];
