%% Notes
%  1. This program reproduces the results for GNJD of Experiment 5 in reference 1. 
...
disp('Speech separation with GNJD');
%% Room 4 or Room 5
switch setting
    case 0
       %% Room 4
...
%         cov_ten = reshape(tgt_ten,[nsrc,nsrc*n_blk,R,R]);
       ...
       %% GNJD
        [Y, G, P1, P2] = f_GNJD(cov_ten,1); %just cross correlation- 2 means columns-parallel
    ...
    elseif fbin == nbin && mod(fbin-1,R)>1
       ...
       %% GNJD
        [Y, G, P1, P2] = f_GNJD(covv_ten,2); %just cross correlation- 2 means columns-parallel
...
cd RESULTS
audiowrite('GNJD_y1.wav',4*Y_smooth(1,:),fs);
audiowrite('GNJD_y2.wav',4*Y_smooth(2,:),fs);
%% computing SIR
cd ..
