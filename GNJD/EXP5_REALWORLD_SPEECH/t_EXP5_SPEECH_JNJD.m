%% Notes
%  1. This program reproduces the results for JNJD of Experiment 5 in reference 2. 
...
disp('Speech separation with JNJD');
%% Room 4 or Room 5
switch setting
    case 0
       %% Room 4
        fftlen = 4096;                % fft length   
        blklen = 7;                   % block length 
        R = 3;                        % number of datasets that are processed each time
    ...
    case 1
       %% Room 5
        ...
        blklen = 1 ;                  % block length 
        R = 4;                        % number of datasets that are processed each time
...
cd GJD_FUNCS
tic
for fbin=2:nbin % generally there is no useful information on the fundamental frequency   
    if mod(fbin-1,R) == 0
       %% Construct the target matrices
        X_mtx = [];
        for rr = R-1:-1:0
            X_mtx = [X_mtx;squeeze(Xfft(:,:,fbin-rr))];
        end
        X_len = size(X_mtx,2);
        overlap = (blklen-1)/blklen;%overlapping coefficient
        n_blk = round((X_len-blklen)/((1-overlap)*blklen)) + 1;%block number 
        ibloc = round((0:n_blk-1)*(X_len-blklen)/(n_blk-1));%the starting subscript array of every block                     
        t_cov = zeros(nsrc*R,nsrc*R,n_blk);
        for kk = 1:n_blk
            temp = X_mtx(:,ibloc(kk)+1:ibloc(kk)+blklen);
            t_cov(:,:,kk) = temp*temp'/blklen;
        end
        tgt_ten = reshape(t_cov,[nsrc,R,nsrc,R,n_blk]);
        tgt_ten = permute(tgt_ten,[1,3,5,2,4]);
%         cov_ten = reshape(tgt_ten,[nsrc,nsrc*n_blk,R,R]);
        tgt_ten = reshape(tgt_ten,[nsrc*nsrc,n_blk,R,R]);
        for r1 = 1:R
            for r2 = 1:R
                [U,S,V] = svd(tgt_ten(:,:,r1,r2),'econ');
                ccov_ten = reshape(U*S,[nsrc,nsrc,nsrc^2]);
                ccov_ten = reshape(ccov_ten,[nsrc,nsrc^3]);   
                cov_ten(:,:,r1,r2) = ccov_ten;
            end
        end
        for rr = 1:R-1
            tgtt_ten((rr-1)*nsrc+1:rr*nsrc,:) = cov_ten(:,:,rr,rr+1);
        end
       %% JNJD
        [G, Y, sub_off, sum_off, sub_diag, sum_diag] = f_JNJD(tgtt_ten, R);
        G = reshape(G,[nsrc,R,nsrc]);
        G = permute(G,[1,3,2]);
        Wfft(:,:,fbin-R+1:fbin) = G;   
        for rr = R-1:-1:0
            Yfft(:,:,fbin-rr) = Wfft(:,:,fbin-rr) * Xfft(:,:,fbin-rr);
        end   
    elseif fbin == nbin && mod(fbin-1,R)==1
       %% LUCJD
    ...
    elseif fbin == nbin && mod(fbin-1,R)>1
        X_mtx = [];
        for rr = mod(fbin-1,R)-1:-1:0
            X_mtx = [X_mtx;squeeze(Xfft(:,:,fbin-rr))];
        end
        X_len = size(X_mtx,2);
        overlap = (blklen-1)/blklen;
        n_blk = round((X_len-blklen)/((1-overlap)*blklen)) + 1; 
        ibloc = round((0:n_blk-1)*(X_len-blklen)/(n_blk-1));       
        RR = mod(fbin-1,R);
        t_cov = zeros(nsrc*RR,nsrc*RR,n_blk);
        for kk = 1:n_blk
            temp = X_mtx(:,ibloc(kk)+1:ibloc(kk)+blklen);
            t_cov(:,:,kk) = temp*temp'/blklen;
        end
        tgt_ten = reshape(t_cov,[nsrc,RR,nsrc,RR,n_blk]);
        tgt_ten = permute(tgt_ten,[1,3,5,2,4]);
        tgt_ten = reshape(tgt_ten,[nsrc*nsrc,n_blk,RR,RR]);
        for r1 = 1:RR
            for r2 = 1:RR
                [U,S,V] = svd(tgt_ten(:,:,r1,r2),'econ');
                ccov_ten = reshape(U*S,[nsrc,nsrc,nsrc^2]);
                ccov_ten = reshape(ccov_ten,[nsrc,nsrc^3]);   
                covv_ten(:,:,r1,r2) = ccov_ten;
            end
        end
        clear tgtt_ten;
        for rr = 1:RR-1
            tgtt_ten((rr-1)*nsrc+1:rr*nsrc,:) = covv_ten(:,:,rr,rr+1);
        end
       %% JNJD
        [G, Y, sub_off, sum_off, sub_diag, sum_diag] = f_JNJD(tgtt_ten, RR);
        G = reshape(G,[nsrc,RR,nsrc]);
        G = permute(G,[1,3,2]);
        Wfft(:,:,fbin-RR+1:fbin) = G;   
        for rr = RR-1:-1:0
            Yfft(:,:,fbin-rr) = Wfft(:,:,fbin-rr) * Xfft(:,:,fbin-rr);
        end   
    end    
...
cd RESULTS
audiowrite('JNJD_y1.wav',4*Y_smooth(1,:),fs);
audiowrite('JNJD_y2.wav',4*Y_smooth(2,:),fs);
%% computing SIR
cd ..
