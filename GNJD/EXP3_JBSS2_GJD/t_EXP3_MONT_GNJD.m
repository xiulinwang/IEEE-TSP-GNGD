...
for snr = 6:10
    ...
    for numm = 1:mont
       ...
       %% Separate
        tic
        [Y1, W(:,:,numm), sub_off, sum_off, sub_diag, sum_diag] = f_JBSS_SOS_GNJD(X,tgt_opt,com_opt,alfa,beta,tau);
        time_(numm) = toc;
    ...
    cd EXP3_JBSS2_GJD
    cd RESULTS
    file_name = ['EXP3_SOS_GNJD_SNR=' num2str(snr) 'dB_N=' num2str(N) '_R=' num2str(R) '_T=' num2str(T) '_MIX=' num2str(mix) '_SRC=' num2str(src) '_NSE=' num2str(nse) '_ROU=' num2str(rou) '_TGT=' num2str(tgt_opt) '.mat'];
