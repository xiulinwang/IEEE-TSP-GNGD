function W = f_JBSS_SOS_LUCJD(X,tgt_opt,alfa,beta,tau)
%% This function do Joint Blind Source Separation 
% with Nonorthogonal-Joint Diagonalization(NJD)---LUCJD and second-order statistics 
%% The inputs:
...
%% References:
%  K. Wang, X.-F. Gong, Q.-H. Lin, "Complex non-orthogonal joint diagonalization based on LU and LQ decompositions" 
...
switch tgt_opt
    case 0
        ...
        t_cov = zeros(N,N,n_blk,R);
        for rr = 1:R
            for kk = 1:n_blk
                temp = X(:,ibloc(kk)+1:ibloc(kk)+blk_snap,rr);
                t_cov(:,:,kk,rr) = temp*temp'/blk_snap;
            end
        end
    case 1
        n_blk = length(tau);
        ...
        for rr = 1:R
            for kk = 1:n_blk
                temp_i = X(:,1:T-tau(kk),rr);
                temp_j = X(:,tau(kk)+1:T,rr);                
                t_cov(:,:,kk,rr) = temp_i*temp_j'/(T-tau(kk));
            end
        end
    case 2
        n_blk = 1;
...
tgt_ten = reshape(t_cov,[N,N*n_blk,R]);

%% Separate with NJD
W = [];
for ii = 1:R
    [Y, A_,C] = f_LUCJD(tgt_ten(:,:,ii),1); 
    W = [W;A_];
end
end
%% Sub-functions GNJD
