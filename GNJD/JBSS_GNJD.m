function [Y, W, P1, P2] = JBSS_GNJD(X,tgt_opt,com_opt,alfa,beta,tau)
%% This function do Joint Blind Source Separation(JBSS) 
...
%     is a fourth order tensor arranged similarly to the input C
...
% Performing 2nd-order J-BSS via GNJD
[Y, W, P1, P2] = f_JBSS_SOS_GNJD(X,tgt_opt,com_opt,alfa,beta,tau);
...
function DEMO_JBSS_GNJD
%% a demo for JBSS via GNJD with second-order statistics

% Constants
N = 3;             % N: dimentionality of target matrices; 
R = 6;             % R: number of multiple datasets
T = 2000;          % T: number of snapshots
snr = 10;          % snr: signal-to-noise ratio in terms of dB
rou = 0.8;         % rou: spatial colorness between adjacent components
tgt_opt = 0;       % 0 - time-varying covariance matrices
com_opt = 0;       % 0 - No compression
alfa = 0.5;        % alfa: the overlapping rate;
beta=200;          % beta : block length
tau=[];            % tau : a vector containing time shifts  

% Construct the multi-set signals
X = f_JBSS_DATA(N, R, T, rou, snr); 

% Perforrming 2nd-order J-BSS via GNJD
[Y, W, P1, P2] = f_JBSS_SOS_GNJD(X, tgt_opt, com_opt, alfa, beta, tau);
...
function X = f_JBSS_DATA(N,R,T,rou,snr)
%% Construct the multi-set signals
%% the inputs
% N: dimentionality of target matrices; 
...
%% the output
% X: the multi-set signals

%--------------------------------------------------------------------------
% BPSK signals with short-time nonstationarity: St--zeros(N,T,R)
%--------------------------------------------------------------------------
Pn=0.01;           % noise level                                                                              
Ps=Pn*10^(snr/10); % signal level
J = sqrt(-1);
...
function [Y, W, P1, P2] = f_JBSS_SOS_GNJD(X,tgt_opt,com_opt,alfa,beta,tau)
%% Performing 2nd-order J-BSS via GNJD
[N,T,R] = size(X);  % N: dimentionality of target matrices; 
                    % R: number of multiple datasets
                    % T: number of snapshots
X_mtx = reshape(permute(X,[1,3,2]),[N*R,T]);
%% Calculate the target matrices
switch tgt_opt
    case 0  % - time-varying covariance matrices
    ...
    case 1 % - time-shifted covariance matrices
        n_blk = length(tau);
        t_cov = zeros(N*R,N*R,n_blk);
        for kk = 1:n_blk
...
elseif com_opt == 1 % - Compression with eigenmatrices scheme
    tgt_ten = reshape(t_cov,[N,R,N,R,n_blk]);
    if R <= 3
        for rrr = 1:R
